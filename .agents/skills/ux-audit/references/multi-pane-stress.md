...
(function detectMinContentCollapse() {
  ...
  const candidates = document.querySelectorAll(
    'main, [role="main"], .timeline, .messages, [data-pane]',
  );
  for (const el of candidates) {
    ...
    if (r.width < 200 && r.height > 100) {
      findings.push({
        ...
        height: r.height.toFixed(1),
        suspect:
          "pane width < 200px on what looks like primary content — verify min-width or fold-trigger",
      });
    }
  }
...
(function discoverPanes() {
  ...
  document
    .querySelectorAll(
      '[data-pane], aside, [role="complementary"], .sidebar, .drawer, .sheet, [data-sidebar], [data-drawer]',
    )
    .forEach((el) => {
      const r = el.getBoundingClientRect();
      if (r.width === 0) return;
      found.push({
        selector:
          el.tagName.toLowerCase() +
          (el.id ? `#${el.id}` : "") +
          (el.dataset.pane ? `[data-pane="${el.dataset.pane}"]` : ""),
        width: r.width.toFixed(1),
