...
Code reading verifies a button exists and has an `onClick`. It does not verify that clicking the button actually **does something observable**. Bugs of this shape — _"the handler runs, fires a call into an SDK, but the flow never completes"_ — are invisible to static analysis and require a live click + network check.
